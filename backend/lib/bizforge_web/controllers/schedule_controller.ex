defmodule BizforgeWeb.ScheduleController do
  use BizforgeWeb, :controller
  require Logger

  alias Bizforge.Repo
  alias Bizforge.Schemas.{Schedule, Agent, Session}
  import Ecto.Query
  ...
  def create(conn, params) do
    ...
    case Repo.insert(changeset) do
      {:ok, schedule} ->
        ...
        if schedule.enabled, do: Bizforge.Scheduler.add_schedule(schedule)
      ...
      schedule ->
        ...
        case Repo.update(changeset) do
          {:ok, updated} ->
            ...
            Bizforge.Scheduler.remove_schedule(updated.id)
            if updated.enabled, do: Bizforge.Scheduler.add_schedule(updated)
  ...
  def delete(conn, %{"id" => id}) do
    case Repo.get(Schedule, id) do
      ...
      schedule ->
        Bizforge.Scheduler.remove_schedule(schedule.id)
        ...
        case schedule
          ...
          {:ok, updated} ->
            Bizforge.EventBus.broadcast(
              Bizforge.EventBus.agent_topic(schedule.agent_id),
            ...
            Task.Supervisor.start_child(Bizforge.HeartbeatRunner, fn ->
              Bizforge.Heartbeat.run(schedule.agent_id,
