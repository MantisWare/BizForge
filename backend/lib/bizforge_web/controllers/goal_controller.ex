defmodule BizforgeWeb.GoalController do
  use BizforgeWeb, :controller
  require Logger

  alias Bizforge.Repo
  alias Bizforge.Schemas.{Goal, Issue}
  import Ecto.Query
    ...
    case Repo.get(Goal, goal_id) do
      ...
      _goal ->
        ...
        Task.Supervisor.start_child(Bizforge.TaskSupervisor, fn ->
          case Bizforge.GoalDecomposer.decompose(goal_id, opts) do
            {:ok, issues} ->
              goal = Repo.get(Goal, goal_id)

              if goal do
                Bizforge.EventBus.broadcast(
                  Bizforge.EventBus.workspace_topic(goal.workspace_id),
                  %{
