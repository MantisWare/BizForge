defmodule BizforgeWeb.ConfigController do
  ...
  def show(conn, _params) do
    config =
      @readable_keys
      |> Enum.map(fn key ->
        val = Application.get_env(:bizforge, key, default_for(key))
        {key, val}
      end)
      |> Map.new()
  ...
  def update(conn, params) do
    ...
    if Enum.empty?(allowed_updates) do
      ...
      Enum.each(allowed_updates, fn {key, value} ->
        Application.put_env(:bizforge, key, coerce(key, value))
      end)

      updated_config =
        @readable_keys
        |> Enum.map(fn key ->
          {key, Application.get_env(:bizforge, key, default_for(key))}
        end)
        |> Map.new()
