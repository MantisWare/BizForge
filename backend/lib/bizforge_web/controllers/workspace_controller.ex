defmodule BizforgeWeb.WorkspaceController do
  use BizforgeWeb, :controller

  alias Bizforge.Repo
  alias Bizforge.Schemas.{Workspace, Agent, Skill, Project}
  import Ecto.Query
  ...
  def create(conn, params) do
    ...
    case Repo.insert(changeset) do
      {:ok, workspace} ->
        init_bizforge_dir(workspace.path)
  ...
  def config(conn, %{"workspace_id" => id}) do
    case Repo.get(Workspace, id) do
      ...
      workspace ->
        ...
        system_path = Path.join(bizforge_dir, "SYSTEM.md")
        company_path = Path.join(bizforge_dir, "COMPANY.md")
  ...
  defp init_bizforge_dir(path) when is_binary(path) do
    ...
    unless File.exists?(system_path) do
      File.write!(system_path, "# System\n\nThis is the Bizforge workspace system prompt.\n")
    end
  end

  defp init_bizforge_dir(_), do: :ok
