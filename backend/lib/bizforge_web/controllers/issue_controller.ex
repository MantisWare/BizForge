defmodule BizforgeWeb.IssueController do
  use BizforgeWeb, :controller

  alias Bizforge.Repo
  alias Bizforge.Schemas.{Issue, Comment, Agent, Label, IssueLabel}
  import Ecto.Query
  ...
  def checkout(conn, %{"issue_id" => id} = params) do
    ...
    case Repo.get(Issue, id) do
      ...
      _issue ->
        case Bizforge.Work.checkout_issue(id, agent_id) do
          {:ok, updated} ->
            updated = Repo.preload(updated, [:labels, :assignee, :comments])
            json(conn, %{issue: serialize(updated)})
  ...
  def dispatch(conn, %{"issue_id" => issue_id}) do
    case Bizforge.IssueDispatcher.dispatch(issue_id) do
      {:ok, :dispatched} ->
