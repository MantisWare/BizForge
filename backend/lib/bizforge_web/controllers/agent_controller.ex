defmodule BizforgeWeb.AgentController do
  use BizforgeWeb, :controller

  alias Bizforge.Repo
  alias Bizforge.Schemas.{Agent, Approval, CostEvent, Session, Schedule}
  import Ecto.Query
  ...
  plug BizforgeWeb.Plugs.Governance, [action: :delete_agent] when action in [:delete, :terminate]
    ...
    case Repo.insert(changeset) do
      {:ok, agent} ->
        Bizforge.EventBus.broadcast(
          Bizforge.EventBus.workspace_topic(agent.workspace_id),
  ...
  def wake(conn, %{"agent_id" => id}) do
    ...
    Enum.each(agent_schedules, &Bizforge.Scheduler.add_schedule/1)
