defmodule BizforgeWeb.WebhookController do
  use BizforgeWeb, :controller

  alias Bizforge.Repo
  alias Bizforge.Schemas.{Webhook, WebhookDelivery}
  import Ecto.Query
    ...
    case Repo.insert(changeset) do
      {:ok, webhook} ->
        Bizforge.EventBus.broadcast(
          Bizforge.EventBus.workspace_topic(webhook.workspace_id),
  ...
  defp verify_secret(conn, %Webhook{secret: secret}) do
    signature = get_req_header(conn, "x-bizforge-signature") |> List.first()
  ...
  defp build_headers(%Webhook{secret: secret}, payload) do
    ...
    [{"x-bizforge-signature", sig}, {"content-type", "application/json"}]
  end
