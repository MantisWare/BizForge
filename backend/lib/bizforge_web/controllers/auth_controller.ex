defmodule BizforgeWeb.AuthController do
  ...
  def login(conn, %{"email" => email, "password" => password}) do
    ...
    cond do
      user && Bcrypt.verify_pass(password, user.password_hash) ->
        {:ok, token, _claims} =
          Bizforge.Guardian.encode_and_sign(user, %{"role" => user.role}, ttl: {1, :hour})
  ...
  def status(conn, _params) do
    with ["Bearer " <> token] <- get_req_header(conn, "authorization"),
         {:ok, claims} <- Bizforge.Guardian.decode_and_verify(token),
  ...
  def refresh(conn, _params) do
    with ["Bearer " <> token] <- get_req_header(conn, "authorization"),
         {:ok, _old_claims} <- Bizforge.Guardian.decode_and_verify(token),
         {:ok, _old_token, {new_token, new_claims}} <- Bizforge.Guardian.refresh(token) do
