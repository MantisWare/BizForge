defmodule Bizforge.Workflows.Engine do
  @moduledoc """
  Workflow execution engine.

  Each `start_run/2` call spawns a dedicated Task under `Bizforge.TaskSupervisor`
  ...
  alias Bizforge.Repo
  alias Bizforge.Schemas.{Workflow, WorkflowStep, WorkflowRun}
  ...
  def start_run(workflow_id, params \\ %{}) do
    with %Workflow{} = workflow <- load_workflow(workflow_id),
      ...
      Task.Supervisor.start_child(Bizforge.TaskSupervisor, fn ->
  ...
  defp execute_step(%WorkflowStep{step_type: "agent_task"} = step, context, _run) do
    ...
    case agent_id do
      ...
      agent_id ->
        ...
        case Bizforge.Heartbeat.run(agent_id,
