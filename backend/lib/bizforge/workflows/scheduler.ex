defmodule Bizforge.Workflows.Scheduler do
  @moduledoc """
  ...
  Quantum dependency that the heartbeat `Bizforge.Scheduler` uses.
  ...
  alias Bizforge.Repo
  alias Bizforge.Schemas.Workflow
  alias Bizforge.EventBus
  import Ecto.Query
  ...
  def handle_cast(:reload, state) do
    ...
    Bizforge.Scheduler.jobs() |> Enum.each(fn {name, _} -> Bizforge.Scheduler.delete_job(name) end)
  ...
  def handle_cast({:deregister, workflow_id}, state) do
    ...
    Bizforge.Scheduler.delete_job(workflow_job_name(workflow_id))
  ...
  defp schedule_cron(%Workflow{} = workflow, cron_expr, timezone, state) do
    ...
    case Crontab.CronExpression.Parser.parse(cron_expr) do
      {:ok, cron} ->
        ...
        Bizforge.Scheduler.delete_job(job_name)
        Bizforge.Scheduler.add_job(job)
  ...
  defp trigger_workflow(workflow_id, trigger_event) do
    ...
    case Bizforge.Workflows.Engine.start_run(workflow_id, %{
