defmodule Bizforge.BudgetEnforcerTest do
  use Bizforge.DataCase

  import Bizforge.TestHelpers

  alias Bizforge.BudgetEnforcer

  setup do
    # Clean ETS tables between tests
    :ets.delete_all_objects(:bizforge_budget_accumulator)
    :ets.delete_all_objects(:bizforge_hierarchy_cache)
    :ok
  end
  ...
  describe "record_cost/1" do
    ...
    test "persists CostEvent to database" do
      ...
      events = Repo.all(from ce in Bizforge.Schemas.CostEvent, where: ce.agent_id == ^agent.id)
  ...
  describe "budget policy enforcement" do
    test "triggers warning incident at warning threshold" do
      ...
      incidents =
        Repo.all(
          from bi in Bizforge.Schemas.BudgetIncident,
    ...
    test "triggers hard_stop and pauses agent at 100%" do
      ...
      updated_agent = Repo.get!(Bizforge.Schemas.Agent, agent.id)
    ...
    test "team-level hard stop pauses all team agents" do
      ...
      assert Repo.get!(Bizforge.Schemas.Agent, agent1.id).status == "paused"
      assert Repo.get!(Bizforge.Schemas.Agent, agent2.id).status == "paused"
    end
  end
  ...
  describe "invalidate_hierarchy/1" do
    test "clears cached hierarchy for agent" do
      ...
      assert :ets.lookup(:bizforge_hierarchy_cache, agent.id) != []
