defmodule BizforgeWeb.Plugs.GovernanceTest do
  use BizforgeWeb.ConnCase

  import Bizforge.TestHelpers

  setup do
    # Ensure governance gates are disabled by default
    Application.put_env(:bizforge, :governance_gates, %{})
    on_exit(fn -> Application.delete_env(:bizforge, :governance_gates) end)
  end
  ...
  describe "when spawn_agent gate is enabled" do
    setup do
      Application.put_env(:bizforge, :governance_gates, %{spawn_agent: true})
      :ok
    end
  ...
  describe "when delete_agent gate is enabled" do
    setup do
      Application.put_env(:bizforge, :governance_gates, %{delete_agent: true})
      :ok
    end
  ...
  describe "when budget_override gate is enabled" do
    setup do
      Application.put_env(:bizforge, :governance_gates, %{budget_override: true})
      :ok
    end
  ...
  describe "non-gated endpoints pass through" do
    setup do
      Application.put_env(:bizforge, :governance_gates, %{spawn_agent: true, delete_agent: true})
      :ok
    end
