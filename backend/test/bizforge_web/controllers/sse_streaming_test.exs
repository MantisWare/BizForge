defmodule BizforgeWeb.SSEStreamingTest do
  @moduledoc """
  ...
  describe "SSE pipeline configuration" do
    test "streaming pipeline is defined in router" do
      ...
      routes = BizforgeWeb.Router.__routes__()
  ...
  describe "SSE EventBus topics" do
    test "workspace_topic builds correct topic string" do
      ...
      assert Bizforge.EventBus.workspace_topic(id) == "workspace:#{id}"
    end

    test "session_topic builds correct topic string" do
      id = Ecto.UUID.generate()
      assert Bizforge.EventBus.session_topic(id) == "session:#{id}"
    end

    test "activity_topic returns global topic" do
      assert Bizforge.EventBus.activity_topic() == "activity:global"
    end

    test "logs_topic returns global topic" do
      assert Bizforge.EventBus.logs_topic() == "logs:global"
    end
  end
end
