<!-- src/lib/components/phases/PhaseDetail.svelte -->
<script lang="ts">
  import type { PhaseTreeNode, Phase } from '$api/types';
  ...
  let statusStyle   = $derived(STATUS_STYLES[phase.status] ?? STATUS_STYLES.active);
  let priorityStyle = $derived(PRIORITY_STYLES[phase.priority] ?? PRIORITY_STYLES.medium);
  let progressColor = $derived(PROGRESS_COLORS[phase.status] ?? '#3b82f6');
  let progressPct   = $derived(Math.min(100, Math.max(0, phase.progress)));
  ...
  async function handleDelete() {
    ...
    deleting = true;
    await phasesStore.deletePhase(phase.id);
    deleting = false;
    onClose();
  }

  async function handleEdit(data: Partial<Phase>) {
    await phasesStore.updatePhase(phase.id, data);
    editing = false;
  }
</script>
...
<aside class="gd-panel" role="dialog" aria-modal="true" aria-label="Phase details: {phase.title}">
  <header class="gd-header">
    <h2 class="gd-title">{phase.title}</h2>
  ...
  <div class="gd-body">
    ...
    {#if phase.description}
      <div class="gd-section">
        ...
        <p class="gd-description">{phase.description}</p>
      </div>
    {/if}
    ...
    <div class="gd-section">
      ...
      <div class="gd-meta-grid">
        <div class="gd-meta-item">
          <span class="gd-meta-key">Sub-phases</span>
          <span class="gd-meta-value">{phase.children.length}</span>
        </div>
        <div class="gd-meta-item">
          <span class="gd-meta-key">Tasks</span>
          <span class="gd-meta-value">{phase.task_count}</span>
        </div>
        <div class="gd-meta-item">
          <span class="gd-meta-key">Created</span>
          <span class="gd-meta-value">{new Date(phase.created_at).toLocaleDateString()}</span>
        </div>
        <div class="gd-meta-item">
          <span class="gd-meta-key">Updated</span>
          <span class="gd-meta-value">{new Date(phase.updated_at).toLocaleDateString()}</span>
        </div>
      </div>
    </div>
    ...
    {#if phase.children.length > 0}
      <div class="gd-section">
        <h3 class="gd-section-label">Sub-phases</h3>
        <div class="gd-subgoals">
          {#each phase.children as child (child.id)}
            <button
              class="gd-subgoal"
              type="button"
              onclick={() => phasesStore.selectPhase(child)}
              aria-label="View sub-phase: {child.title}"
            >
  ...
  <footer class="gd-footer">
    <button class="gd-action-btn" type="button" onclick={() => editing = true} aria-label="Edit phase">
...
{#if editing}
  <PhaseForm phase={phase} onSubmit={handleEdit} onCancel={() => editing = false} />
{/if}
