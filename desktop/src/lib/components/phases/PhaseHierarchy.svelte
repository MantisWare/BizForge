<!-- src/lib/components/phases/PhaseHierarchy.svelte -->
<script lang="ts">
  import type { PhaseTreeNode } from '$api/types';
  import PhaseCard from './PhaseCard.svelte';
  import PhaseHierarchy from './PhaseHierarchy.svelte';
...
<div class="gh-level" role={depth === 0 ? 'tree' : 'group'} aria-label={depth === 0 ? 'Phases hierarchy' : undefined}>
  {#each nodes as node (node.id)}
    <div class="gh-node" style="--depth: {Math.min(depth, MAX_VISUAL_DEPTH)}" role="treeitem" aria-selected={phasesStore.selected?.id === node.id} aria-expanded={node.children.length > 0 ? expanded[node.id] ?? true : undefined}>
      {#if depth > 0}
      ...
      <div class="gh-item">
        {#if node.children.length > 0}
          <button
            ...
            aria-label="{expanded[node.id] ?? true ? 'Collapse' : 'Expand'} sub-phases for {node.title}"
            type="button"
          >
            <svg
              width="12"
        ...
        <div class="gh-card-wrap">
          <PhaseCard phase={node} onSelect={(p) => phasesStore.selectPhase(p)} />
        </div>
      </div>

      {#if node.children.length > 0 && (expanded[node.id] ?? true) && depth < MAX_VISUAL_DEPTH}
        <div class="gh-children">
          <PhaseHierarchy nodes={node.children} depth={depth + 1} />
        </div>
      {:else if node.children.length > 0 && (expanded[node.id] ?? true) && depth >= MAX_VISUAL_DEPTH}
        <div class="gh-max-depth" role="note" aria-label="Maximum nesting depth reached">
          <span>{node.children.length} more sub-phases (max depth reached)</span>
        </div>
      {/if}
    </div>
  {/each}
