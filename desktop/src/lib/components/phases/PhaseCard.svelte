...
<script lang="ts">
  import type { PhaseTreeNode } from '$api/types';
  import { phasesStore } from '$lib/stores/phases.svelte';

  interface Props {
    phase: PhaseTreeNode;
    onSelect?: (phase: PhaseTreeNode) => void;
  }

  let { phase, onSelect }: Props = $props();
  ...
  async function handleDecompose(e: MouseEvent) {
    e.stopPropagation();
    decomposing = true;
    await phasesStore.decompose(phase.id);
    decomposing = false;
  }
...
<article
  class="gc-card"
  aria-label="Phase: {phase.title}, {progressPct}% complete"
  onclick={() => onSelect?.(phase)}
  onkeydown={(e) => e.key === 'Enter' && onSelect?.(phase)}
  tabindex={onSelect ? 0 : undefined}
  style={onSelect ? 'cursor: pointer' : undefined}
>
  <header class="gc-header">
    <h3 class="gc-title">{phase.title}</h3>
    <div class="gc-badges">
      <span
        class="gc-badge"
      ...
      <span
        ...
        aria-label="Priority: {phase.priority}"
      >
        {phase.priority}
      </span>
    </div>
  ...
  <footer class="gc-footer">
    {#if phase.children.length > 0}
      <span class="gc-meta-item" aria-label="{phase.children.length} sub-phases">
        <svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" aria-hidden="true">
          <path d="M3 3h18M3 12h9M3 21h6" />
        </svg>
        {phase.children.length} sub-phases
      </span>
    {/if}
    {#if phase.task_count > 0}
      <span class="gc-meta-item" aria-label="{phase.task_count} tasks">
        <svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" aria-hidden="true">
    ...
    <div class="gc-footer-right">
      ...
      <button
        ...
        aria-label="Decompose phase into tasks"
        type="button"
      >
        {#if decomposing}
