...
<script lang="ts">
  ...
  interface Props {
    ...
    facingZ?: number;
    charState?: 'idle' | 'walk' | 'type' | 'sleep';
    facingAngle?: number;
    onclick: () => void;
  }

  let { agent, position, selected, emissive, zoneColor = '#8888a0', teamColor, divisionColor, deskType = 'desk', facingZ = 1, charState = 'idle', facingAngle = 0, onclick }: Props = $props();

  const isConference = $derived(deskType === 'conference');

  const SKIN_TONES = ['#f5d0a9', '#e8b887', '#d4956b', '#b87a52', '#8b5e3c', '#6b4226'];
  ...
  function agentSkinTone(id: string): string {
    return SKIN_TONES[djb2(id) % SKIN_TONES.length];
  }

  const isWalking = $derived(charState === 'walk');
  const isActive = $derived(agent.status === 'running' || agent.status === 'idle');

  let bobY = $state(0);
  let walkBounce = $state(0);
  let walkLean = $state(0);

  useTask(() => {
    const now = Date.now();
    if (isWalking) {
      walkBounce = Math.abs(Math.sin(now * 0.008)) * 0.12;
      walkLean = Math.sin(now * 0.006) * 0.08;
      bobY = 0;
    } else if (isActive) {
      bobY = Math.sin(now * 0.003) * 0.05;
      walkBounce = 0;
      walkLean = 0;
    } else {
      bobY = 0;
  ...
  const skinTone = agentSkinTone(agent.id);
...
<T.Group position.x={position[0]} position.y={position[1]} position.z={position[2]}>
  {#if isWalking}
    <!-- ═══ WALKING MODE — character only, no furniture ═══ -->
    <T.Group rotation.y={facingAngle}>
      <!-- Walking character with bounce -->
      <T.Group position.y={0.55 + walkBounce}>
        <!-- Body -->
        ...
        <!-- Walking legs (two cylinders swinging) -->
        <T.Mesh position={[-0.06, -0.32 + walkLean * 0.3, 0]} rotation.x={walkLean}>
          <T.CylinderGeometry args={[0.05, 0.04, 0.3, 6]} />
        ...
        <!-- Walking shadow on ground -->
        <T.Mesh position.y={-0.54} rotation.x={-Math.PI / 2}>
          <T.CircleGeometry args={[0.25, 16]} />
          <T.MeshBasicMaterial color="#000000" transparent opacity={0.15} />
        </T.Mesh>

        <!-- Status ring while walking -->
        {#if selected}
      ...
      <!-- Name label above walking character -->
      <T.Group position.y={1.55}>
  ...
  {:else}
    <!-- ═══ SEATED MODE — furniture + character ═══ -->
    {#if !isConference}
