    ...
    handler: (_path, options) => {
      if ((options.method ?? "GET").toUpperCase() === "POST") {
        ...
        const newIssue: Issue = {
          ...
          depends_on_ids: (body.depends_on_ids as string[]) ?? [],
          task_type: (body.task_type as Issue["task_type"]) ?? null,
          execution_order: (body.execution_order as number | null) ?? null,
          created_at: now,
          updated_at: now,
        };
        addIssue(newIssue);
  ...
  },
  {
    ...
    handler: (path) => {
      ...
      return {
        status: "started",
        phase_id: id,
        message: "Phase decomposition started. Tasks will appear shortly.",
      };
    },
  },
  {
